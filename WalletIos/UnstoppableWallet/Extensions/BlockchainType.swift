...
extension BlockchainType {
    ...
    var imageUrl: String {
        ...
        if (uid == "gala"){
            let imageName = "\(uid)_32"
            print("imageName :--- ",imageName)
            let imag = UIImage(named: imageName)
            let url = fetchImageURL(fromAssetsNamed: imageName)
            print("imageName url :--- ",url)
            return url
//            if let imagePath = Bundle.main.path(forResource: "gala_32", ofType: "png") {
//                let imageUrl = URL(fileURLWithPath: imagePath)
//                print("Image URL: \(imageUrl)")
//                return "\(imageUrl)"
//            } else {
//                print("Image not found")
//                return "https://cdn.blocksdecoded.com/blockchain-icons/32px/fantom@\(scale)x.png"
//            }
...
//            return imageURLStr
    ...
    func supports(accountType: AccountType) -> Bool {
        ...
        case .evmPrivateKey, .evmAddress:
            switch self {
            case .ethereum, .binanceSmartChain, .polygon, .avalanche, .optimism, .arbitrumOne, .gnosis, .fantom,.gala: return true
            default: return false
            }
        case .tronAddress:
            return self == .tron
    ...
    var description: String {
        ...
        case .gala: return "GALA, ERC20 token"
        default: return ""
        }
    }
    ...
    var brandColor: UIColor? {
        ...
        case .gala : return UIColor(hex: 0x96BEDC)
        default: return nil
        }
    }
...
func fetchImageURL(fromAssetsNamed imageName: String) -> String {
    // Try to load the image from the asset catalog
    if let image = UIImage(named: imageName) {
        // Convert the UIImage to PNG data
        if let imageData = image.pngData() {
            // Create a temporary directory and file URL for the image
            let tempDirectory = FileManager.default.temporaryDirectory
            let imageURL = tempDirectory.appendingPathComponent("\(imageName).png")
            ...
            if !FileManager.default.fileExists(atPath: imageURL.path) {
                ...
                print("File already exists at path: \(imageURL.path)")
            }
