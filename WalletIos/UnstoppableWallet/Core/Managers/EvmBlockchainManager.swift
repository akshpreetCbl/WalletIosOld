...
class EvmBlockchainManager {
    ...
    var allBlockchains: [Blockchain] {
        print("allBlockchains EvmBlockchainManager.blockchainTypes",EvmBlockchainManager.blockchainTypes.map { $0.uid })
      
        do {
            try print ("marketKit.allBlockchains() :-- ",marketKit.allBlockchains())
